...
describe('useConstructorArgs', () => {
  ...
  test('passes address as param', async () => {
    ...
    mockRpc({ edb_getConstructorArgs: (p) => { received = p!; return '0xdeadbeef'; } });
    ...
    expect(result.current.data).toBe('0xdeadbeef');
  });

  test('schema error on bad shape', async () => {
    mockRpc({ edb_getConstructorArgs: () => ({ unexpected: 'object' }) });
    const { wrapper } = makeWrapper();
  ...
  test('does not fetch when addr is undefined', async () => {
    let calls = 0;
    mockRpc({ edb_getConstructorArgs: () => { calls++; return null; } });
    const { wrapper } = makeWrapper();
